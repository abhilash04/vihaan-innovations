...
const StyledIconCircle = styled(Box)(({ bgcolor, active, animation }) => ({
  ...
  ...(active && { animation: `${animation} 1.5s infinite ease-in-out` }),
}));
...
const LaunchIcon = ({ active }) => (
  <IconWrapper active={active}>
    <svg viewBox="0 0 24 24" fill="currentColor">
      <path d="M13.13,22.19L11.5,18.35c-0.12-0.28-0.34-0.5-0.62-0.62l-3.84-1.63L21,6L13.13,22.19z" />
    </svg>
  </IconWrapper>
);
...
// Mobile step list
const MobileStepList = ({ steps, activeStep, hoverIdx, setHoverIdx }) => (
  <Box sx={{ width: "100%", px: 2 }}>
    {steps.map((step, idx) => {
      const isCardActive = hoverIdx === -1 ? activeStep === idx : hoverIdx === idx;
      return (
        <Box
          key={step.id}
          ...
          sx={{
            ...
            background: isCardActive
              ? "linear-gradient(135deg, rgba(0,210,255,0.15), rgba(58,123,213,0.1))"
              : "rgba(255,255,255,0.05)",
            border: isCardActive ? "1px solid rgba(0,210,255,0.4)" : "1px solid rgba(255,255,255,0.08)",
            transition: "all 0.4s ease",
            cursor: "pointer",
          }}
        >
          <StyledIconCircle bgcolor={step.color} active={isCardActive} animation={step.func}>
            {React.cloneElement(step.icon, { active: isCardActive })}
          </StyledIconCircle>
          <Typography
            sx={{
              color: isCardActive ? "#00d2ff" : "rgba(255,255,255,0.7)",
              fontWeight: 700,
              fontSize: "14px",
...
const OrderCycle = () => {
  ...
  const animate = (time) => {
    ...
    if (sectionProgress < orbitRatio) {
      ...
      if (ringRefs.current[sectionIdx]) {
        ringRefs.current[sectionIdx].setAttribute("stroke-dashoffset", 339 - (339 * orbitT));
      }
    } else {
      active = -1;
    ...
    if (active !== activeStep) setActiveStep(active);
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <Box sx={{ textAlign: "center", mb: isMobile ? 2 : 8, maxWidth: "800px", zIndex: 10, px: isMobile ? 2 : 0 }}>
        <Typography
          sx={{
        ...
        >
          Experience a smooth cycle of strategy, creation, and expansion designed to grow your
          business with continuous single vendor ecommerce development.
        </Typography>
      </Box>

      {/* Desktop animated orbit diagram */}
      {!isMobile ? (
        <Box sx={{ position: "relative", width: 1000, height: 450 }}>
          ...
          {steps.map((step, idx) => (
            ...
            >
              ...
              <StyledIconCircle bgcolor={step.color} active={activeStep === idx} animation={step.func}>
              ...
              >
                <Typography
                  sx={{ color: "#fff", fontWeight: 900, fontSize: "15px", textTransform: "uppercase", letterSpacing: "1.5px", lineHeight: 1 }}
                >
                  {step.label}
                </Typography>
              </Box>
        ...
        <MobileStepList steps={steps} activeStep={activeStep} hoverIdx={hoverIdx} setHoverIdx={setHoverIdx} />
      )}

      {/* CTA Button */}
      <Box sx={{ mt: isMobile ? 6 : 12, zIndex: 10 }}>
        <Button
          variant="contained"
