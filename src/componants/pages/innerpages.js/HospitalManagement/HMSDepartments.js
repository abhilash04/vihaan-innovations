...
const depts = [
  { icon: FavoriteIcon, title: "Cardiology", color: "#EF4444", features: ["Outpatient Triage", "ECG Syncing", "Vitals history"] },
  { icon: AccessibilityNewIcon, title: "Orthopaedics", color: "#3B82F6", features: ["Joint replacement", "Trauma care", "Physio desk"] },
  { icon: PregnantWomanIcon, title: "Gynaecology", color: "#EC4899", features: ["Prenatal checkups", "IVF tracking", "Delivery desk"] },
  { icon: ChildCareIcon, title: "Paediatrics", color: "#10B981", features: ["Vaccination logs", "Growth charts", "Neonatal care"] },
  { icon: CoronavirusIcon, title: "Oncology", color: "#8B5CF6", features: ["Chemo schedules", "Tumor boards", "Radiation Sync"] },
  { icon: VisibilityIcon, title: "Ophthalmology", color: "#06B6D4", features: ["Vision testing", "LASIK layouts", "Optical store"] },
  { icon: CleanHandsIcon, title: "Dental Care", color: "#F59E0B", features: ["Orthodontics", "Root canal", "X-Ray mount"] },
  { icon: PsychologyIcon, title: "Psychiatry", color: "#6366F1", features: ["Counseling stats", "Therapy logs", "Medication desk"] }
];
      ...
      <Container maxWidth="lg">
        <Box sx={{ textAlign: "center", mb: 6 }}>
          ...
          <Typography variant="h2" sx={{ fontWeight: 800, color: "#0B2046", fontSize: { xs: "28px", md: "36px" }, mb: 2, fontFamily: "Plus Jakarta Sans, sans-serif" }}>
            Covering Every Department in Your Hospital
          </Typography>
        </Box>
                ...
                <motion.div custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={flipVariants}>
                  <Paper elevation={0} sx={{ p: 0, borderRadius: "16px", bgcolor: "#ffffff", border: "1px solid rgba(0,0,0,0.04)", overflow: "hidden", height: "100%", display: "flex", flexDirection: "column", transition: "all 0.3s ease", boxShadow: "0 4px 15px rgba(0,0,0,0.02)", "&:hover": { transform: "translateY(-5px)", boxShadow: "0 12px 30px rgba(10,45,110,0.06)", "& .color-band": { height: "50px" }, "& .learn-more": { transform: "translateY(0)", opacity: 1 }, "& .icon-circle": { transform: "scale(1.1)" } } }}>
                    ...
                    <Box sx={{ p: 2.5, flex: 1, display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
                      ...
                      <Typography sx={{ color: "#0B2046", fontWeight: 800, fontSize: "15px", mb: 1.5 }}>{dept.title}</Typography>
                      ...
                      <List sx={{ p: 0, mb: 2, width: "100%" }}>
                        {dept.features.map((f, k) => (
                          <ListItem key={k} sx={{ p: 0, mb: 0.5, justifyContent: "center" }}>
                            ...
                            <ListItemText primary={f} primaryTypographyProps={{ sx: { color: "rgba(0,0,0,0.5)", fontSize: "11px", textAlign: "left" } }} />
                          </ListItem>
                        ))}
                      </List>
