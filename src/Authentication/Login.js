            ...
            <Slider {...settings}>
              {images.map((image, index) => (
                <Box key={index} sx={{ position: "relative", mt: -20 }}>
                  ...
                  >
                    {index === 0 && "Ajji's Receipe - Optimize Your Online Presence"}
                    {index === 1 && "Ajji's Receipe - Maximize Your Search Ranking"}
                    {index === 2 && "Ajji's Receipe - Boost Your Website Traffic"}
                  </Typography>
                </Box>
              ))}
            </Slider>
